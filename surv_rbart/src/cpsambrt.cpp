...
)
{
   ...
   for(size_t i=0;i<burn;i++) {
      ...
      if(i % printevery==0) COUT << "At iteration " << i << " of " << burn << " for burn, where f(1)=" << ambm.f(1) << endl;
   }
   if(summarystats) {
      ambm.setstats(true);
      psbm.setstats(true);
   ...
   for(size_t i=0;i<nd;i++) {
      if((i % printevery) ==0) COUT << "At iteration " << i << " of " << nd << " for draws, where f(1)=" << ambm.f(1) << endl;
      ambm.draw(gen);
      dips = di;
      dips -= fambm;
      psbm.draw(gen);
